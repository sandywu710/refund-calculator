...
function calcRefund(
  ...
  const penalty =
    isFault && refundRate > 0 && refundRate < 1
      ...
      : 0;
  const actualRefund = Math.max(0, refundBase - penalty);
        ...
        {result && (
          <div className="mt-5 bg-white rounded-2xl shadow-sm border border-gray-200 p-6 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Metric
                label="已過天數"
                value={`${result.elapsedDays} 天`}
                sub={`期間比例 ${result.periodPct.toFixed(1)}%`}
              />
              <Metric
                label="已使用次數"
            ...
            <div className="text-center text-sm text-gray-600 bg-gray-50 rounded-lg py-2">
              ...
              <span className="font-semibold text-gray-800">
                {" "}{result.effectivePct.toFixed(1)}%
              </span>
            </div>

            <div
              className={`rounded-lg border-l-4 px-4 py-3 text-sm font-medium ${
                ruleColor[result.rule] ?? "bg-gray-50 border-gray-400 text-gray-800"
              }`}
            >
              <div className="font-bold mb-0.5">{result.rule}</div>
              <div className="font-normal opacity-80">{result.ruleDetail}</div>
            </div>
              ...
              <div className="border-t pt-3 flex justify-between items-center">
                ...
                <span
                  className={`text-2xl font-bold ${
                    result.actualRefund > 0 ? "text-blue-600" : "text-red-500"
                  }`}
                >
                  NT$ {fmt(result.actualRefund)}
                </span>
              </div>
            </div>
          </div>
        ...
        <details className="mt-5 bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
          ...
          <ul className="mt-3 space-y-2 text-sm text-gray-600">
            {[
              ...
              "規則三：逾 10% 未逾 30% → 退還 80%（可歸責學員另扣 20% 違約金）",
              "規則四：逾 30% 未逾 50% → 退還 50%（可歸責學員另扣 20% 違約金）",
              "規則五：逾 50% → 不退款",
            ].map((r, i) => (
...
function Row({
  ...
  return (
    <div className="flex justify-between items-center text-sm">
      ...
      <span
        className={
          red ? "text-red-500 font-medium" : "text-gray-800 font-medium"
        }
      >
        {value}
      </span>
